...
Original file is located at
    https://colab.research.google.com/drive/13q0nFlGOjyukdFgfEJhvjIS7IuCYVKUx
...
print(mean_squared_error(y_true=y_train, y_pred=rfc_tunning.predict(X_train)))
print(mean_squared_error(y_true=y_test, y_pred=rfc_tunning.predict(X_test)))
...
pickle.dump(rfc_tunning, open('model_rfc.pkl', 'wb'))

location = './model_rfc.pkl'
file = open(location, "rb")
model = pickle.load(file)

data1 = pd.DataFrame(data1, columns=numerical_features)
data2 = pd.DataFrame(data2, columns=numerical_features)

prediction1 = model.predict(data1)
prediction2 = model.predict(data2)
# predict with scaler
prediction_ss1 = model.predict(pd.DataFrame(scaler.transform(data1), columns=numerical_features))
prediction_ss2 = model.predict(pd.DataFrame(scaler.transform(data2), columns=numerical_features))

print(prediction1)
print(prediction2)
print(prediction_ss1)
print(prediction_ss2)

model_dict = {'DecisionTree': decisiontree, 'SVM': svm, 'RandomForestClassification': rfc, 'KNeighborsClassification': knnc, 'AdaBoostClassifier':adaboost, 'RFC_GridSearchCV': rfc_tunning}
prediksi = X_test.iloc[30:35].copy()
pred_dict = {'y_true':y_test[30:35]}
for name, model in model_dict.items():
    pred_dict['prediksi_'+name] = model.predict(prediksi)
